...
public class KnowledgeDocumentIngestionService {
    ...
    public KnowledgeDocumentIngestionService(
            ...
            @Value("${sms.ai.knowledge-ingestion.postgres.url:jdbc:postgresql://localhost:5432/sms-dev}") String vectorUrl,
            @Value("${sms.ai.knowledge-ingestion.postgres.username:postgres}") String vectorUsername,
            @Value("${sms.ai.knowledge-ingestion.postgres.password:postgres123}") String vectorPassword,
            @Value("${sms.ai.knowledge-ingestion.postgres.table:ai_knowledge_chunk}") String vectorTable) {
        this.sourceJdbcTemplate = sourceJdbcTemplate;
        this.embeddingService = embeddingService;
        this.objectMapper = objectMapper;
        this.vectorTable = safeIdentifier(vectorTable, "ai_knowledge_chunk");
    ...
    public IngestionResult ingest(IngestionRequest request) {
        ...
        int indexRevision = request == null || request.indexRevision() == null ? 1 : request.indexRevision();
        indexRevision = 1;
        if (indexRevision <= 0) throw new IllegalArgumentException("indexRevision must be positive");

        List<StagingDetail> details = loadDetails(1L,
               1L, limit);
...
//        }
    ...
    private String safeIdentifier(String value, String fallback) {
        String identifier = value == null || value.isBlank() ? fallback : value.trim();
