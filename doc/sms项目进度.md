...
| 指标 | 当前评估 | 说明 |
|---|---:|---|
| 企业级 AI 功能覆盖度 | **40%** | 已有 AI 编排、工具、RAG 数据模型和 JWT/RBAC 骨架，缺少企业级闭环。 |
| 工程基础完成度 | **65%** | 多模块结构、分层、持久化、配置治理和基础编译验证已具备。 |
| 生产就绪度 | **20%** | 尚缺真实依赖联调、鉴权边界、可观测性、测试矩阵、部署与容灾。 |
| 本阶段目标 | **已完成** | 已完成 AI 分层、会话链路优化、配置统一和 `sms-web` 基础鉴权。 |

### 企业级能力完成度

| 能力域 | 完成度 | 当前状态 |
|---|---:|---|
| 架构分层与配置治理 | **75%** | `sms-ai` 已分为 application/domain/infrastructure/interfaces；配置统一从 `.env` 绑定。 |
...
| 认证、RBAC 与数据隔离 | **40%** | `sms-web` 已有 JWT、会话、角色、按钮权限；AI 服务尚未纳入认证上下文，未实现租户隔离。 |
| 前端产品闭环 | **30%** | 登录契约已转至 `sms-web`；SSE 聊天、动态菜单、知识库和运营页面尚未接真实后端。 |
...
- 已实现登录、刷新令牌、JWT 过滤、无状态会话校验和用户状态校验。
...
- 后端运行参数统一由根目录 `.env` 提供，YAML 仅保留配置结构和变量绑定。
- 环境变量已去除 `SMS_` 前缀；`.env.example` 为非敏感参数提供样例，敏感值保持空白。
...
| 差距 | 当前情况 | 企业级目标 |
|---|---|---|
| AI 服务鉴权 | `sms-ai` 聊天接口仍接收客户端传入的 `userId`、`sessionId`。 | 从 JWT/网关上下文取得用户、租户、角色和数据范围。 |
| 租户隔离 | 没有 `tenantId` 数据模型、过滤器或行级数据隔离。 | 所有会话、知识库、工具、日志和权限按租户隔离。 |
...
| 差距 | 当前情况 | 企业级目标 |
|---|---|---|
| 向量维度 | pgvector DDL 为 `VECTOR(1024)`，默认 embedding 配置为 `1356`。 | 统一模型、配置和表结构维度，并在启动/入库时校验。 |
...
- 登录后动态菜单、角色切换、按钮权限、刷新令牌失效处理尚未与真实后端闭环。
...
| 令牌会话 | 数据库存储完整 access/refresh token。 | 存储哈希或 token 标识，支持会话注销、轮换和设备管理。 | P1 |
...
1. 将 `sms-ai` 接入 JWT 鉴权或统一网关，禁止客户端直接传递可信身份；为 run、会话、知识库和工具结果增加租户与用户范围校验。
...
```text
mvn -pl sms-ai,sms-web,sms-student/sms-student-provider,sms-knowledge/sms-knowledge-provider -am -DskipTests compile
mvn -pl sms-web -am -Dtest=JwtTokenServiceTest -Dsurefire.failIfNoSpecifiedTests=false test
pnpm.cmd exec vue-tsc --noEmit
```

已验证的是编译、JWT 声明和前端类型；尚未验证真实 MySQL、Redis、Nacos、模型服务、PostgreSQL/pgvector、Milvus 的运行时集成行为。

## 8. 阶段验收标准

- 用户可通过 `sms-ui` 登录 `sms-web`，令牌刷新、注销和权限变更按预期生效。
- AI 请求只能从认证上下文获取用户和租户，无法跨用户或跨租户读取 run、会话或知识库数据。
