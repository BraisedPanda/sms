...
| 指标 | 当前评估 | 说明 |
|---|---:|---|
| 企业级 AI 功能覆盖度 | **43%** | 已有 AI 编排、工具、RAG 数据模型和跨服务 JWT/RBAC 鉴权，仍缺企业级闭环。 |
| 工程基础完成度 | **68%** | 多模块结构、分层、持久化、配置治理、共享认证能力和定向验证已具备。 |
| 生产就绪度 | **25%** | 已建立 AI 鉴权边界，仍缺真实依赖联调、数据隔离、可观测性、测试矩阵、部署与容灾。 |
| 本阶段目标 | **已完成** | 已完成 AI 分层、会话链路优化、配置统一及 Web/AI 共享 JWT 鉴权。 |
...
| 认证、RBAC 与数据隔离 | **55%** | Web/AI 已共享 Auth0 JWT、会话、角色和按钮权限；租户数据模型、行级过滤和权限运营尚未实现。 |
...
- `sms-ai` 已接入 Spring Security，`/api/ai/**` 必须携带有效 Access Token；聊天运行的用户和会话身份仅从 JWT 上下文获取。
...
| 跨服务会话失效 | `sms-web` 会校验持久化会话，`sms-ai` 当前仅校验 JWT 签名、类型和过期时间。 | 经网关或共享会话校验实现注销、禁用和权限变更的跨服务即时生效。 |
...
| 令牌会话 | 数据库存储完整 access/refresh token，AI 尚未校验共享会话状态。 | 存储哈希或 token 标识，并经网关或共享会话服务支持注销、轮换、设备管理和即时失效。 | P1 |
...
1. 为 run、会话、知识库和工具结果增加租户与用户范围校验；确定网关透传规范或接入共享会话校验，使注销、禁用和权限变更能跨服务即时生效。
...
```text
mvn -pl sms-common-core test
mvn -pl sms-ai,sms-web -am -DskipTests compile
...
已验证的是共享 JWT 声明、AI/Web 编译和前端类型；尚未验证真实 MySQL、Redis、Nacos、模型服务、PostgreSQL/pgvector、Milvus 的运行时集成行为，也尚未进行 AI 鉴权的 HTTP 集成测试。
